...
ESX.ShowColoredNotification = function(message, colorID)
	Citizen.InvokeNative(0x92F0DA1E27DB96DC, tonumber(colorID))
...
ESX.ShowInventory = function()
	...
	if ESX.PlayerData.money > 0 then
		...
		table.insert(elements, {
			label     = ('%s: <span style="color:#2ecc71;padding: 0 0 0 8px;">%s</span>'):format(_U('cash'), formattedMoney),
	...
	for k,v in pairs(ESX.PlayerData.accounts) do
		if v.money > 0 and v.name ~= 'bank' then
			...
			table.insert(elements, {
				label     = ('%s: <span style="color:#2ecc71;padding: 0 0 0 8px;">%s</span>'):format(ESX.PlayerData.accounts[i].label, formattedMoney),
				count     = v.money,
				type      = 'item_account',
				value     = v.name,
				usable    = false,
...
AddEventHandler('esx:serverCallback', function(requestId, ...)
	ESX.ServerCallbacks[requestId](...) -- ici
...
RegisterNetEvent('marzouz:showColoredNotification')
AddEventHandler('marzouz:showColoredNotification', function(message, colorID)
	ESX.ShowColoredNotification(message, colorID)
end)
