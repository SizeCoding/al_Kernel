...
---== Menu pause : affichage du nom du serveur
Citizen.CreateThread(function()
	AddTextEntry('FE_THDR_GTAO', '~b~Libe~s~rty~r~City~s~ - RP')
end)
...
if Config.DontShootInVehicle then
				...
				if currentPos == prevPos then
					if time > 0 then
						if kickWarning and time == math.ceil(secondsUntilKick / 4) then
							TriggerEvent('esx:showAdvancedNotification', "Douanier", "~r~Expulsion / Absent", "Vous allez être ramené à la douane ~y~dans ".. time .. " secondes ~r~!", "CHAR_ORTEGA", 8)
						end
						time = time - 1
					else
	...
	Citizen.CreateThread(function()
		while true do
			...
			if DoesEntityExist(GetVehiclePedIsTryingToEnter(PlayerPedId())) then
				...
				local volvehicle = math.random(1000)

				if pedd then
					if volvehicle ~= 50 then
...
local DiscordAppId = tonumber(GetConvar("RichAppId", "539911370179739658"))
local DiscordAppAsset = GetConvar("RichAssetId", "al")

local timeWaiting = 15000
Citizen.CreateThread(function()
	while true do
		SetDiscordAppId(DiscordAppId)
		SetDiscordRichPresenceAsset(DiscordAppAsset)
		Citizen.Wait(timeWaiting)
		if ESX.PlayerData.nom and ESX.PlayerData.prenom then
			SetRichPresence(ESX.PlayerData.prenom .. " " .. ESX.PlayerData.nom .. " ID : ["..GetPlayerServerId(PlayerId()).."]")
			SetDiscordRichPresenceAssetText('Serveur RP Francophone')
			SetDiscordRichPresenceAssetSmall('info')
			SetDiscordRichPresenceAssetSmallText('Rejoignez nous sur discord : https://discord.gg/fGEEGPh')
			break
		else
			SetRichPresence("Vient d'atterrir à Los Santos. ID : ["..GetPlayerServerId(PlayerId()).."]")
			SetDiscordRichPresenceAssetText('Serveur RP Francophone')
			SetDiscordRichPresenceAssetSmall('info')
			SetDiscordRichPresenceAssetSmallText('Rejoignez nous sur discord : https://discord.gg/fGEEGPh')
		end
