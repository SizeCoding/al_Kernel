...
MySQL.ready(function()
	...
	---### GANG
	local result_gang = MySQL.Sync.fetchAll('SELECT * FROM gangs', {})

	for i=1, #result_gang do
		ESX.Gangs[result_gang[i].name] = result_gang[i]
		ESX.Gangs[result_gang[i].name].grades = {}
	end

	local result2_gang = MySQL.Sync.fetchAll('SELECT * FROM gang_grades', {})

	for i=1, #result2_gang do
		if ESX.Gangs[result2_gang[i].gang_name] then
			ESX.Gangs[result2_gang[i].gang_name].grades[tostring(result2_gang[i].grade)] = result2_gang[i]
		else
			print(('es_extended: invalid job "%s" from table gang_grades ignored!'):format(result2_gang[i].job_name))
		end
	end

	for ke,va in pairs(ESX.Gangs) do
		if next(va.grades) == nil then
			ESX.Gangs[va.name] = nil
			print(('es_extended: ignoring gang "%s" due to missing job grades!'):format(va.name))
		end
	end
end)
