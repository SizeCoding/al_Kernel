...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb)
		MySQL.Async.execute('UPDATE users SET job = @job, job_grade = @job_grade, gang = @gang, gang_grade = @gang_grade, loadout = @loadout, position = @position WHERE identifier = @identifier', {
...
ESX.SavePlayers = function(cb)
	...
	Async.parallelLimit(asyncTasks, 8, function(results)
		RconPrint('[SAUVEGARDE] Tous les joueurs' .. "\n")
...
ESX.RegisterServerCallback('fiveplayers:getItemAmount', function(source, cb, item)
